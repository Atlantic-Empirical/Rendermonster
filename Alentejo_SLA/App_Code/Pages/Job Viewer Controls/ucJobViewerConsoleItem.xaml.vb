...
Partial Public Class ucJobViewerConsoleItem
    ...
    Private Function JobMessageToString(ByRef m As cSMT_ATJ_JobProgressMessage) As String
        Select Case m._Type
            ...
            Case 2
                Dim bm As String = ""
                If String.IsNullOrEmpty(m.Benchmark) Then
                    If IsNumeric(m.Benchmark) Then
                        If m.Benchmark > 0 Then bm = m.Benchmark
                    Else
                        bm = m.Benchmark ' "N/A"
                    End If
                End If
                Return "Sample Level=" & m.SampleLevel & bm & If(Not String.IsNullOrEmpty(m.Message), " Elapsed=" & m.Message, "")
            Case Else
                Return ""
        End Select
    End Function
