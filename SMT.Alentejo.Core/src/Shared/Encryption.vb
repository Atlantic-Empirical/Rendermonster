...
#Region "  Asymmetric"
    ...
    Public Class Asymmetric
        ...
        Private _KeyContainerName As String = "Encryption.AsymmetricEncryption." & Guid.NewGuid.ToString
...
#End Region
        ...
        Public Sub Dispose() Implements IDisposable.Dispose
            If _rsa IsNot Nothing Then _rsa.Clear()
        End Sub
        ...
        Private Function DecryptPrivate(ByVal encryptedData As Data) As Data
            Try
                Debug.WriteLine("DecryptPrivate()")
                Dim d As New Data(_rsa.Decrypt(encryptedData.Bytes, False))
                _rsa.Clear()
                Return d
            Catch ex As Exception
                Throw New Exception("Problem with DecryptPrivate(). Error: " & ex.Message)
            End Try
        End Function
...
'                Return d
'            Catch ex As Exception
'                Throw New Exception("Problem with Decrypt(). Error: " & ex.Message)
'            End Try
'        End Function
...
'        ''' Generates a new public/private key pair as objects
'        ''' </summary>
'        Public Shared Sub GenerateNewKeyset(ByRef publicKey As PublicKey, ByRef privateKey As PrivateKey)
'            Dim PublicKeyXML As String = Nothing
'            Dim PrivateKeyXML As String = Nothing
'            GenerateNewKeyset(PublicKeyXML, PrivateKeyXML)
'            publicKey = New PublicKey(PublicKeyXML)
...
'        ''' Generates a new public/private key pair as XML strings
'        ''' </summary>
'        Public Shared Sub GenerateNewKeyset(ByRef publicKeyXML As String, ByRef privateKeyXML As String)
'            Dim rsa As RSA = RSACryptoServiceProvider.Create
'            publicKeyXML = rsa.ToXmlString(False)
'            privateKeyXML = rsa.ToXmlString(True)
'        End Sub
...
'        ''' </summary>
'        Public Sub New(ByVal s As String)
'            If String.IsNullOrEmpty(s) Then Throw New Exception("Invalid string (empty).")
'            Text = s
'        End Sub
...
'            Get
'                If _b Is Nothing OrElse _b.Length = 0 Then Return ""
'                Return Convert.ToBase64String(_b)
'            End Get
'            Set(ByVal Value As String)
'                Try
'                    If String.IsNullOrEmpty(Value) Then Exit Property
'                    _b = Convert.FromBase64String(Value)
'                Catch ex As System.FormatException
'                    Throw New System.FormatException("The provided string does not appear to be Base64 encoded:" & Environment.NewLine & Value & Environment.NewLine, ex)
'                End Try
'            End Set
'        End Property
